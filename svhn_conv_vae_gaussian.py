...
def adv_test(orig_img = 0, target_img = 1, C = 100.0, plot = True):
    ...
    adv_img = adv_plot(np.tile(test_x[orig_img], (batch_size, 1, 1, 1)).reshape(batch_size, 3, 32, 32))[0]
...
C = [0, 0.5, 1, 10, 50, 100, 200, 300, 500, 1000, 2500, 10000]
for c in C:
    od, ad = adv_test(0,1, C=c, plot = True) #2231,3113
    orig_dist.append(od)
    adv_dist.append(ad)

plt.figure()
plt.plot(np.log(C), orig_dist, label = "Original image and adversarial image distance")
plt.plot(np.log(C), adv_dist, label = "Target image and adversarial reconstruction distance")
plt.ylabel("L2 norm")
